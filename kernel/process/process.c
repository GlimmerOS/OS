...
void processSetup(struct Process* process)
{
   acquire(&process->lock);
    ...
    {
        Assert(0,"processInitFail:pagetable disalloc");
    }
     if((process->trapframe = (struct Trapframe *)alloc_physic_page()) == 0){
         Assert(0,"processInitFail:trapFrame disalloc");
        }
        memset(process->trapframe,0,sizeof(process->trapframe));
...
void
processPgtableAlloc(struct Process *process)
{
  // An empty page table.
  ...
  if(va_map_pa(process->pagetable, TRAMPOLINE, (uint64_t)trampoline, MPTE_FLAG(R)|MPTE_FLAG(W)) < 0){
    va_unmap_pa(process->pagetable,TRAMPOLINE);
    free_physic_page(process->pagetable);
    Assert(0,"TRAMPOLINE map failed");
  }

  if(va_map_pa(process->pagetable, TRAPFRAME, (uint64_t)(process->trapframe), MPTE_FLAG(R)|MPTE_FLAG(W)) < 0){
    va_unmap_pa(process->pagetable, TRAMPOLINE);
