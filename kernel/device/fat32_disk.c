...
{
  if(i >= NUM)
    Assert(0, "free_desc 1");
  if(disk.free[i])
    Assert(0, "free_desc 2");
  disk.desc[i].addr = 0;
  disk.desc[i].len = 0;
  disk.desc[i].flags = 0;
  disk.desc[i].next = 0;
...
virtio_disk_rw(struct buf *b, int write)
{
  ...
  *VIRTIO0_MMIO_REG(VIRTIO_MMIO_QUEUE_NOTIFY) = 0; // value is queue number
...
virtio_disk_intr()
{
  ...
  *VIRTIO0_MMIO_REG(VIRTIO_MMIO_INTERRUPT_ACK) = *VIRTIO0_MMIO_REG(VIRTIO_MMIO_INTERRUPT_STATUS) & 0x3;
  ...
  while(disk.used_idx != disk.used->idx){
    ...
    if(disk.info[id].status != 0)
      Assert(0, "virtio_disk_intr status");
