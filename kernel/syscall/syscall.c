...
int
getStrFmAddr(uint64_t addr, char *buf, int max)
{
...
void syscall(void)
{
    ...
    if(num>0 &&syscalls[num]) //here num can up to an uncorret height,it will be wrong,not now
    {
        process->trapframe->a0=syscalls[num]();
    }
    else{
        printf("%d %s :unknown syscall,syscall num:%d",process->processID,process->name,num);
